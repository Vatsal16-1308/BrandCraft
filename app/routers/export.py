...
logger = logging.getLogger("bizforge.export")
...
async def generate_brand_bible(data: BrandGuideRequest):
    ...
    try:
        ...
        logger.info(f"Generated brand guide PDF for '{data.brand_name}' ({len(buffer.getvalue())} bytes)")
        ...
        return StreamingResponse(
            BytesIO(b"PDF generation failed"),
            status_code=500,
